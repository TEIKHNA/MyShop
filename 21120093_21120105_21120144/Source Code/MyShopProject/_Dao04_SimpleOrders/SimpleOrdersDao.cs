...
namespace _Dao04_SimpleOrders
{
    public class SimpleOrdersDao : OrdersIDao
    {
        public override int setCustomerToOrder(int? cusId, int ordId)
        {
            ...
            if (cusId != 0 && cusId != null) command.Parameters.Add("@cusId", System.Data.SqlDbType.Int).Value = cusId;
            ...
            using (var reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    var customer = new Customer()
                    {
                        ...
                        Tel = Convert.IsDBNull(reader["Tel"]) ? "" : (string)reader["Tel"],
                        Address = Convert.IsDBNull(reader["Address"]) ? "" : (string)reader["Address"],
        ...
        public override int addOneOrderDetail(OrderDetail detail)
        {
            ...
            command.Parameters.Add("@pricePerItem", System.Data.SqlDbType.Int).Value = detail.PricePerItem == null ? 1 : detail.PricePerItem;
        ...
        public override Tuple<int, BindingList<Order>> getOrders(int skip, int take, DateTime? startDate = null, DateTime? endDate = null)
        {
            ...
            {
                condition = "WHERE CreatedAt >= @startDate AND CreatedAt <= @endDate";
            }
            var sql = @"
        ...
        public override int minusProductQuantity(int? proId)
        {
            ...
            string sql = @"
                ...
                WHERE ProId = @proId AND Quantity > 0;
            ";
            var command = new SqlCommand(sql, DBInstance.Instance.Connection);
