            ...
            do
            {
                ...
                if (catIdCell?.InnerText.Length > 0)
                {
                    ...
                    string author = stringTable.SharedStringTable
                            .ElementAt(int.Parse(stringId))
                            .InnerText;
                    int publishedYear = int.Parse(publishedYearCell!.InnerText);
                    int quantity = int.Parse(quantityCell!.InnerText);
                    int originalPrice = int.Parse(originalPriceCell!.InnerText);
                    int sellingPrice = int.Parse(sellingPriceCell!.InnerText);
